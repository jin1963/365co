// app.js (ethers v5) - 365df MLM DApp V3
(() => {
  "use strict";
  ...
  let selectedPkg = null; // 0/1/2 (CoreV3 expects enum value)
  ...
  function detectInjected() {
    // Most wallets use window.ethereum
    if (window.ethereum) return window.ethereum;
    // Some old Binance builds use window.BinanceChain
  ...
  async function approveAndBuy() {
    try {
      ...
      if (selectedPkg === null) return setStatus("Please select a package.");
  ...
  async function refresh() {
    try {
      ...
      // Binary volumes (read from BinaryV3)
      try {
